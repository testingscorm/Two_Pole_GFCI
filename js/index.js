...
function updateDepth(book, newPage) {
	...
	var page = book.turn('page'),
		...
		depthWidth = 10*Math.min(1, page*2/pages);
	...
	if (newPage>3)
		$('.sj-book .p2 .depth').css({
			width: depthWidth,
			left: -800 - depthWidth //sen change to 20 if you have small content pages
		});
	else
		$('.sj-book .p2 .depth').css({width: 0});

		depthWidth = 10*Math.min(1, (pages-page)*2/pages);


    //Sen Last Before page for DEPTH
	
	if (newPage<pages-2)
		$('.sj-book .p27 .depth').css({
			width: depthWidth,
			right: -800 - depthWidth //sen change to 20 if you have small content pages
		});
	else
...
function loadPage(page) {
	...
	$.ajax({url: 'pages/page' + page + '.html'}).
			...
			// sen default $('.sj-book .p' + page).html(pageHtml.replace('samples/steve-jobs/', ''));			
...
function addPage(page, book) {
	...
	if (!book.turn('hasPage', page)) {
		...
		var element = $('<div />',
			{'class': 'own-size',
			//css: {width: 460, height: 582} //Sen Default value for small pages
			css: {width: 630, height: 475} //sen fullpage like cover
