//-------------------Senthil added for self maximized
...
function getSCOid()
{
  ...
  var scoid = loc.substring(loc.length-11, loc.length-5);
  //alert(scoid)
  return scoid;
...
function MillisecondsToCMIDuration(n) 
{
  ...
  var milliseconds = parseInt((duration%1000)/100)
    ...
    var hms = hours + ":" + minutes + ":" + seconds + "." + milliseconds;  
    return hms
...
function shell_DoFSCommand(command, args){
	var shellObj = g_bIsInternetExplorer ? shell : document.shell;
	...
	if (cmd.substring(0,3) == "LMS"){
		...
		if ( cmd == "LMSInitialize" ){
			...
			if ( cmd == "LMSGetValue") {
				shellObj.SetVariable(arg2, SCOGetValue(arg1));
			}	else if ( cmd == "LMSGetLastError") {
				shellObj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetErrorString") {
				shellObj.SetVariable(arg2, SCOGetLastError(arg1));
			}	else if ( cmd == "LMSGetDiagnostic") {
				shellObj.SetVariable(arg2, SCOGetDiagnostic(arg1));
			}	else {
				// for unknown LMSGetxxxx extension
				v = eval('g_objAPI.' + cmd + '(\"' + arg1 + '\")');
				shellObj.SetVariable(arg2,v);
			}
